...
<head>
    <title>Admin Table Tempat | MisiDok - Web Kesehatan & Janji Dokter</title>
...
<div class="container">
    ...
    <div class="mt-3">
        <h2>Table Tempat</h2>
        <div class="mt-4">
            <h5><a href="{{url('/admin/tempat/create')}}">Create Data</a></h5>
            <table class="table table-hover">
                <thead>
                    <tr>
                        <th scope="col">#</th>
                        <th scope="col">Foto</th>
                        <th scope="col" class="text-nowrap">Nama Tempat</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <th scope="row">{{$i++}}</th>
                        <td><img src="{{asset('storage/asset/tempat/'.$item->foto)}}" alt="{{$item->foto}}" class="rounded tempat"></td>
                        <td>{{$item->nama_tempat}}</td>
                        <td>{{$item->alamat}}</td>
                        <td>{{$item->telepon}}</td>
                        <td>{{$item->fax}}</td>
                        <td><a href="{{url('/admin/tempat/update/'.$item->id)}}" class="btn btn-success">Edit</a></td>
                        <td>
                            <form action="{{ url('/admin/tempat/delete/'. $item->id) }}" method="POST">
                                @method('DELETE')
                                @csrf
